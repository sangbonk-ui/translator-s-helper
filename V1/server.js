...
async function loadGlossary() {
  ...
  for (const e of all) {
    ...
    const senses = new Set(
      cur.target.split('/').map((s) => s.trim()).filter(Boolean)
    );
    e.target.split('/').map((s) => s.trim()).filter(Boolean).forEach((s) => senses.add(s));
...
async function checkGlossary(sourceParas, targetParas, glossary) {
  ...
  for (const entry of glossary) {
    ...
    sourceParas.forEach((p, i) => {
      if (contains(p, S)) srcHits.push({ index: i, text: p });
    });

    // 원문에 해당 용어가 없으면 검사 대상 아님
    if (srcHits.length === 0) {
      results.push({ entry, status: 'na', srcHits: [], tgtHits: [] });
      continue;
    }
    ...
    if (entry.type === 'term') {
      ...
      else if (tgtHits.length < srcHits.length) status = 'warn'; // 일부만 적용
      else status = 'pass';
      results.push({ entry, status, srcHits, tgtHits, expected: T });
    } else {
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      const alignedPairs = [];
      for (let i = 0; i < n; i++)
        alignedPairs.push({ src: srcUnits[i] || '', tgt: tgtUnits[i] || '' });
      ...
      for (const r of out.results) {
        ...
        r.matchCount = matches.length;
        // 상한 없음 — 해당 문장 모두. ok=false(기대 번역어 미사용)가 수정 대상.
        r.pairs = matches.map((p) => ({
          src: p.src,
          tgt: p.tgt,
